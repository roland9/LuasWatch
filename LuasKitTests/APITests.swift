...
public struct WrongAPIWorker: APIWorker {
  ...
  public func getTrains(shortCode: String) async throws -> Data {
    ...
    let url = URL(
      string:
        "https://WRONGluasforecasts.rpa.ie/xml/get.ashx?action=forecast&stop=\(shortCode)&encrypt=false"
    )!
...
class APITests: XCTestCase {
  ...
  func testWrongAPI() async {
    ...
    do {
      ...
      XCTAssertEqual(
        error.localizedDescription, "A server with the specified hostname could not be found.")
    }
  }
  ...
  func testMockAPI_RanelaghTrains() async throws {
    ...
    XCTAssertEqual(
      trains.inbound[0], Train(destination: "Broombridge", direction: "Inbound", dueTime: "Due"))
    XCTAssertEqual(
      trains.inbound[1], Train(destination: "Broombridge", direction: "Inbound", dueTime: "5"))

    XCTAssertEqual(trains.outbound.count, 3)
    XCTAssertEqual(
      trains.outbound[0], Train(destination: "Bride's Glen", direction: "Outbound", dueTime: "7"))
    XCTAssertEqual(
      trains.outbound[1], Train(destination: "Sandyford", direction: "Outbound", dueTime: "9"))
    XCTAssertEqual(
      trains.outbound[2], Train(destination: "Bride's Glen", direction: "Outbound", dueTime: "15"))
  ...
  func testMockAPI_ServerError() async {
    ...
    do {
      ...
      XCTAssertEqual(
        (error as NSError), NSError(domain: NSURLErrorDomain, code: NSURLErrorTimedOut))
    }
  }
