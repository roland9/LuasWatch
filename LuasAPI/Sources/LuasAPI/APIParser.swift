...
struct APIParser {
  ...
  static let logger = Logger(subsystem: "LuasAPI", category: "APIParser")
  ...
  class MessageParser: NSObject, NodeParser {
    ...
    func parser(_ parser: XMLParser, foundCharacters string: String) {
      logger.debug("📄 \(String( describing: string))")
    ...
    func parser(
      ...
      logger.debug("📄 \(self.classForCoder) message: \(String( describing: self.message))")
      result = message
      delegateStack?.pop()
    }
  }
  ...
  class DirectionParser: NSObject, NodeParser {
    ...
    func parser(
      ...
      logger.debug("🔛 \(self.classForCoder) parsing \(elementName) attributeDict \(attributeDict)")
  ...
  public static func parse(xml: Data, for trainStation: TrainStation) throws -> TrainsByDirection {
    ...
    if xmlParser.parse() {
      ...
      throw APIError.invalidXML(
        "Error parsing XML: " + (xmlParser.parserError?.localizedDescription ?? ""))
    }
  }
}
